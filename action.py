...
def get_welcome_response():
    ...
    session_attributes = {}
    card_title = "Welcome"
    speech_output = "Welcome to the First Aid. " \
                    "Please tell me how I can help you"
    # If the user either does not reply to the welcome message or says something
    # that is not understood, they will be prompted again with this text.
    reprompt_text = "You can say 'Help me with': "\
...
def create_cpr_output(intent, session):
    ...
    if session['attributes']['compressions']:
        response += "Perform 30 chest compressions. "
    ...
    if 'cpr' in session['attributes']:
        if 'Stage' in intent['slots'].keys():
            ...
            if value == 'compressions':
                ...
                session['attributes']={}
                return create_output("Are you sure you want to stop?", intent, session)
            elif value == 'rcompressions':
                session['attributes']['compressions'] = True
                return create_cpr_output(intent, session)
            elif value == 'rbreaths':
                session['attributes']['compressions'] = False
                ...
                session['attributes']={}
                return create_output("Quitting cpr", intent, session)
            elif value == 'no':
                return create_cpr_output(intent, session)
